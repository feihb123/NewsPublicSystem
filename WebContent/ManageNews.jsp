...
<body style="background-color:#fffff0;background-image: url(1.png);background-position:0% 100%;">
          ...
          当前时间： <b id="ctime">
           ...
           <h2>本部门发布的新闻</h2>     
            <hr> 
            <form action="DeptSearch.jsp?id=<%=request.getParameter("id")%>"  method="post"  style="padding-top:-700px;">
            <center>
            ...
            <%
             ResultSet rs=null;
             Base.getConnection();
             rs=Base.selectSql("select newsID,newTitle from News,DeptUser where News.deptID=DeptUser.deptID and DuserID='"+request.getParameter("id")+"' order by newWeight DESC");
             while(rs.next()){
            %> 
            <tr>
             <td>
